...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	 ...
	 */
	//默认构造函数,初始化一个空容器,容器不包含任何bean信息,需要稍后通过调用其register()方法注册配置类
	//并调用refresh()方法刷新容器,触发容器对注解bean的载入,解析和注册
	 ...
	 */
	//最常用的构造函数,通过将涉及的配置类传递给构造函数,实现将相应配置类的bean自动注册到容器中
	 ...
	 * @see #refresh()
	 */
	//为容器注册一个要被处理的的注解Bean,新注册的Bean,必须手动调用容器的refresh()方法刷新容器,
	//触发容器对注册的bean的处理
